...
function TodoList() {
  ...
  const {loading, error, todolist, setTodolist, createTodoList} = useGet()
  ...
  const handleSubmit = (e) => {
    ...
    createTodoList(newTodo).then(setValue(''))
  }
  ...
  const handleEdit = (id,value) => {
    ...
    if(value) {
      console.log('hello: ', todolist.slice(0,updateTodoIndex).concat([{id: id, title: value, isComplete: false}]).concat(todolist.slice(updateTodoIndex+1)))
      setTodolist(todolist.slice(0,updateTodoIndex).concat([{id: id, title: value, isComplete: false}]).concat(todolist.slice(updateTodoIndex+1)))
      //setTodolist([...todolist.slice(0,updateTodoIndex), {id: id, title: value, isComplete: false}, ...todolist.slice(updateTodoIndex+1)])
      axios.patch(`http://localhost:3005/todolist/${id}`,{id:id, title:value, isComplete: false})
    } else {
      // setTodolist([...todolist.slice(0,updateTodoIndex), ...todolist.slice(updateTodoIndex+1)])
