...
function PrintList() {                    //print todo list to html
  // clear();
  unorderList2.innerHTML = ""
  todoApi.getAllTodos().then((value) => {
    value.todos.forEach((element,index) => {
      unorderList2.appendChild(createListItem(element,index));
    });
  });

  unorderList.innerHTML = unorderList2


  // unorderList.innerHTML = todoApi.getAllTodos().then((value) => {
  //   value.todos.forEach((element,index) => {
  //     return createListItem(element,index);
  //   });
  // });
};
PrintList();